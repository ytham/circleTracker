...
while(cap.isOpened()):
	...
	if ret==True:
		...
		for (x,y,w,h) in detected:
			...
			i = i + 1
			print str(i) + " x,y / w,h: " + str(x) + "," + str(y) + " / " + str(w) + "," + str(h)
			# print str(i) + " DetectedLoopBegin: " + str(pupilFrame[0].size) + "\t" + str(pupilO[0].size)
			cv2.rectangle(frame, (x,y), ((x+w),(y+h)), (0,0,255),1)
			cv2.line(frame, (x,y), ((x+w,y+h)), (0,0,255),1)
			cv2.line(frame, (x+w,y), ((x,y+h)), (0,0,255),1)
			# print "DetectedLined: " + str(pupilFrame[0].size) + "\t" + str(pupilO[0].size)
			...
			#
			# 	if MAindex < contours.count:
			# 		del contours[MAindex]		#remove the picture frame contour
			# 	if MAindex < distanceX.count:
			...
			# 		break
			# 	cx,cy = int(center['m10']/center['m00']), int(center['m01']/center['m00'])
			# 	print "     2: " + str(cx) + ", " + str(cy)
			# 	cv2.circle(pupilO,(cx,cy),5,255,-1)

		#create overlay
		# cv2.rectangle(frame, (dx,dy), ((dx+dw),(dy+dh)), (0,0,255),3)
